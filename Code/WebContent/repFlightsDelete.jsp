	...
	try {
		...
		while(result.next()){
			...
			if(result.getString("airlineID").equalsIgnoreCase(repAirlineID) && 
					result.getString("flightNumber").equalsIgnoreCase(repFlightNumber)){
		...
		if(match == 1){
			out.print("Deleting flight from database...");
			out.print("<br>");
			
			//removing flight from database
			String delete = "DELETE FROM flights WHERE airlineID = '" + repAirlineID + "' AND flightNumber = \""+ repFlightNumber+"\"";
			...
			PreparedStatement fliesOnPS = con.prepareStatement(fliesOnDelete);
			//maybe you cant delete the parent first? ... yup
			fliesOnPS.executeUpdate();
			ps.executeUpdate();
			
			out.print("flight removed from database...");
			out.print("<br>");
		} else{
			out.print("<br>");
			out.print("No deletion. No such flight exists.");
			out.print("<br>");
		...
		con.close();
		out.print("<br>");
		out.print("flight delete page succeeded");
		out.print("<br>");
		
		
	} catch (Exception ex) {
		out.print(ex);
		out.print("flight delete page failed");
	}
%>
